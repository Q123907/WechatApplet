 ...
 * chooseInvoiceTitle 选择用户的发票抬头。当前小程序必须关联一个公众号，且这个公众号是完成了微信认证的，才能调用
 * chooseInvoiceTitle。//微信已不需要手动授权后调用
...
export default class Authorization {
  ...
  async hasModal(modalObj = {}) {
    const modal = modalObj.customModal.show
      ...
      : await this.wxShowModal(modalObj.content)
  ...
  async getSetting(authorization = this.authorization) {
    ...
    this.loading = true
    const res = await wxPromisify(wx.getSetting)().catch(err => err)
    this.loading = false
